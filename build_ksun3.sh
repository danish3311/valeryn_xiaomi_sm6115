...
# --- Compat shim #8: another deep one, in the same family as rules.c above
# but touching policydb's internal data structures directly instead of
# selinux_state.
#
# security/selinux/sepolicy.c errors: "assigning to 'struct avtab_node *'
# from incompatible type 'struct flex_array'" / "incomplete type 'struct
# filename_trans_key'" / "no member named 'stypes'" / "no member named
# 'type_val_to_struct'" (compiler suggested 'role_val_to_struct', which is
# wrong — the real field just has a different name, see below).
#
# Root cause: SELinux's policydb internals went through several rounds of
# restructuring across kernel versions — replacing "struct flex_array"-based
# storage with plain arrays (~5.1+), converting filename_trans from a simple
# hashtable-of-otype into a richer filename_trans_key/stypes-bitmap model
# (~5.7+, then hashtab_key_params at 5.9+), and renaming
# type_val_to_struct_array to type_val_to_struct in the process. dev-susfs
# hardcodes only the newest form of all three, unconditionally.
#
# Verified directly against this kernel's actual policydb.h/avtab.h (not
# assumed): struct avtab.htable is "struct flex_array *" (confirmed, this
# kernel predates 5.8's flex_array removal), struct filename_trans_datum
# has only "otype" — no stypes/next (confirmed, predates 5.7's key/stypes
# model), db->filename_trans is "struct hashtab *" (not a raw hashtab
# struct), db->type_val_to_struct_array and db->type_attr_map_array and
# sym_val_to_name are all "struct flex_array *" (confirmed — the real field
# is literally named type_val_to_struct_array, not type_val_to_struct;
# the compiler's "did you mean role_val_to_struct" was a red herring from
# fuzzy-matching, not the actual fix). Also confirmed this kernel is not the
# Huawei/HISI variant (which uses a third, ebitmap-based branch for these
# same fields) — it uses the standard flex_array layout.
#
# One function, remove_avtab_node(), doesn't exist in the working
# KernelSU-Next-susfs-3.2.0 source at all (genuinely new in dev-susfs) —
# rewritten here using the exact flex_array_get_ptr()/flex_array_put_ptr()
# calls this kernel's own security/selinux/ss/avtab.c already uses for the
# same field, not guessed. The other three functions restore
# KernelSU-Next-susfs-3.2.0's own pre-5.1/5.7/5.8 branches, verified against
# this kernel's real hashtab_search()/hashtab_insert() signatures (3-arg,
# no key_params — also confirmed directly) and adapted to dev-susfs's more
# defensive current error-handling style rather than copied verbatim.
python3 - << 'PYEOF'
path = "drivers/kernelsu/selinux/sepolicy.c"
with open(path) as f:
    content = f.read()

if "pre-5.8 compat path" in content:
    print("sepolicy.c already patched, skipping")
else:
    replacements = []

    old_f1 = 'static bool remove_avtab_node(struct policydb *db, struct avtab_node *node)\n{\n    int i;\n    int ret;\n    int shrink_size = sizeof(struct avtab_key) + sizeof(struct avtab_datum);\n    struct avtab removed = {};\n    struct avtab_node *n;\n    struct avtab_node *prev;\n\n    ret = avtab_alloc(&removed, 1);\n    if (ret < 0)\n        return false;\n\n    for (i = 0; i < db->te_avtab.nslot; i++) {\n        prev = NULL;\n        for (n = db->te_avtab.htable[i]; n; prev = n, n = n->next) {\n            if (n != node)\n                continue;\n\n            if (prev)\n                prev->next = n->next;\n            else\n                db->te_avtab.htable[i] = n->next;\n\n            if (db->te_avtab.nel > 0)\n                db->te_avtab.nel--;\n\n            if ((n->key.specified & AVTAB_XPERMS) && n->datum.u.xperms) {\n                shrink_size += sizeof(u8) + sizeof(u8) + sizeof(u32) * ARRAY_SIZE(n->datum.u.xperms->perms.p);\n            }\n            n->next = NULL;\n            removed.htable[0] = n;\n            removed.nel = 1;\n            avtab_destroy(&removed);\n            if (db->len >= shrink_size)\n                db->len -= shrink_size;\n            return true;\n        }\n    }\n\n    avtab_destroy(&removed);\n    return false;\n}\n'
    new_f1 = 'static bool remove_avtab_node(struct policydb *db, struct avtab_node *node)\n{\n    int i;\n    int ret;\n    int shrink_size = sizeof(struct avtab_key) + sizeof(struct avtab_datum);\n    struct avtab removed = {};\n    struct avtab_node *n;\n    struct avtab_node *prev;\n\n    ret = avtab_alloc(&removed, 1);\n    if (ret < 0)\n        return false;\n\n    /* --- pre-5.8 compat path: this kernel\'s struct avtab uses\n     * "struct flex_array *htable" (not a plain struct avtab_node **),\n     * so slots must go through flex_array_get_ptr()/flex_array_put_ptr()\n     * instead of direct array indexing — same API this kernel\'s own\n     * security/selinux/ss/avtab.c already uses for the same field. --- */\n    for (i = 0; i < db->te_avtab.nslot; i++) {\n        prev = NULL;\n        for (n = flex_array_get_ptr(db->te_avtab.htable, i); n; prev = n, n = n->next) {\n            if (n != node)\n                continue;\n\n            if (prev) {\n                prev->next = n->next;\n            } else {\n                if (flex_array_put_ptr(db->te_avtab.htable, i, n->next,\n                                       GFP_KERNEL | __GFP_ZERO)) {\n                    pr_err("remove_avtab_node: flex_array_put_ptr failed\\n");\n                    avtab_destroy(&removed);\n                    return false;\n                }\n            }\n\n            if (db->te_avtab.nel > 0)\n                db->te_avtab.nel--;\n\n            if ((n->key.specified & AVTAB_XPERMS) && n->datum.u.xperms) {\n                shrink_size += sizeof(u8) + sizeof(u8) + sizeof(u32) * ARRAY_SIZE(n->datum.u.xperms->perms.p);\n            }\n            n->next = NULL;\n            if (flex_array_put_ptr(removed.htable, 0, n, GFP_KERNEL | __GFP_ZERO)) {\n                pr_err("remove_avtab_node: flex_array_put_ptr (removed) failed\\n");\n                avtab_destroy(&removed);\n                return false;\n            }\n            removed.nel = 1;\n            avtab_destroy(&removed);\n            if (db->len >= shrink_size)\n                db->len -= shrink_size;\n            return true;\n        }\n    }\n\n    avtab_destroy(&removed);\n    return false;\n}\n\n'
    replacements.append(("remove_avtab_node", old_f1, new_f1))

    old_f2 = 'static bool add_filename_trans(struct policydb *db, const char *s,\n                               const char *t, const char *c, const char *d,\n                               const char *o)\n{\n    struct type_datum *src, *tgt, *def;\n    struct class_datum *cls;\n    struct filename_trans_key *new_key = NULL;\n    int rc;\n\n    src = symtab_search(&db->p_types, s);\n    if (src == NULL) {\n        pr_warn("source type %s does not exist\\n", s);\n        return false;\n    }\n    tgt = symtab_search(&db->p_types, t);\n    if (tgt == NULL) {\n        pr_warn("target type %s does not exist\\n", t);\n        return false;\n    }\n    cls = symtab_search(&db->p_classes, c);\n    if (cls == NULL) {\n        pr_warn("class %s does not exist\\n", c);\n        return false;\n    }\n    def = symtab_search(&db->p_types, d);\n    if (def == NULL) {\n        pr_warn("default type %s does not exist\\n", d);\n        return false;\n    }\n\n    struct filename_trans_key key;\n    key.ttype = tgt->value;\n    key.tclass = cls->value;\n    key.name = (char *)o;\n\n    struct filename_trans_datum *last = NULL;\n\n    struct filename_trans_datum *trans = policydb_filenametr_search(db, &key);\n    while (trans) {\n        if (ebitmap_get_bit(&trans->stypes, src->value - 1)) {\n            // Duplicate, overwrite existing data and return\n            trans->otype = def->value;\n            return true;\n        }\n        if (trans->otype == def->value)\n            break;\n        last = trans;\n        trans = trans->next;\n    }\n\n    if (trans == NULL) {\n        trans = (struct filename_trans_datum *)kcalloc(1, sizeof(*trans),\n                                                       GFP_KERNEL);\n        if (!trans) {\n            pr_err("add_filename_trans: alloc filename_trans_datum failed\\n");\n            goto out;\n        }\n        new_key = (struct filename_trans_key *)kzalloc(sizeof(*new_key), GFP_KERNEL);\n        if (!new_key) {\n            pr_err("add_filename_trans: alloc filename_trans_key failed\\n");\n            goto free_trans;\n        }\n        *new_key = key;\n        new_key->name = kstrdup(key.name, GFP_KERNEL);\n        if (!new_key->name) {\n            pr_err("add_filename_trans: kstrdup name failed\\n");\n            goto free_key;\n        }\n        trans->next = last;\n        trans->otype = def->value;\n        rc = hashtab_insert(&db->filename_trans, new_key, trans, filenametr_key_params);\n        if (rc) {\n            pr_err("add_filename_trans: hashtab_insert failed: %d\\n", rc);\n            goto free_name;\n        }\n    }\n\n    db->compat_filename_trans_count++;\n    return ebitmap_set_bit(&trans->stypes, src->value - 1, 1) == 0;\n\nfree_name:\n    kfree(new_key->name);\nfree_key:\n    kfree(new_key);\nfree_trans:\n    kfree(trans);\nout:\n    return false;\n}\n\n'
    new_f2 = 'static bool add_filename_trans(struct policydb *db, const char *s,\n                               const char *t, const char *c, const char *d,\n                               const char *o)\n{\n    struct type_datum *src, *tgt, *def;\n    struct class_datum *cls;\n    struct filename_trans *new_key = NULL;\n    struct filename_trans_datum *trans;\n\n    src = symtab_search(&db->p_types, s);\n    if (src == NULL) {\n        pr_warn("source type %s does not exist\\n", s);\n        return false;\n    }\n    tgt = symtab_search(&db->p_types, t);\n    if (tgt == NULL) {\n        pr_warn("target type %s does not exist\\n", t);\n        return false;\n    }\n    cls = symtab_search(&db->p_classes, c);\n    if (cls == NULL) {\n        pr_warn("class %s does not exist\\n", c);\n        return false;\n    }\n    def = symtab_search(&db->p_types, d);\n    if (def == NULL) {\n        pr_warn("default type %s does not exist\\n", d);\n        return false;\n    }\n\n    /* --- pre-5.7 compat path: this kernel\'s struct filename_trans_datum\n     * only has "otype" (no "stypes" ebitmap, no "next" chain — those were\n     * added when filename_trans moved to filename_trans_key + per-key\n     * chaining). Restored from KernelSU-Next-susfs-3.2.0\'s own <5.7.0\n     * branch: a plain hashtab keyed by struct filename_trans, one datum per\n     * key, with participation tracked globally via filename_trans_ttypes\n     * instead of a per-key stypes bitmap. --- */\n    struct filename_trans key;\n    key.ttype = tgt->value;\n    key.tclass = cls->value;\n    key.name = (char *)o;\n\n    trans = hashtab_search(db->filename_trans, &key);\n\n    if (trans == NULL) {\n        trans = (struct filename_trans_datum *)kcalloc(1, sizeof(*trans),\n                                                       GFP_KERNEL);\n        if (!trans) {\n            pr_err("add_filename_trans: alloc filename_trans_datum failed\\n");\n            goto out;\n        }\n        new_key = (struct filename_trans *)kzalloc(sizeof(*new_key), GFP_KERNEL);\n        if (!new_key) {\n            pr_err("add_filename_trans: alloc filename_trans key failed\\n");\n            goto free_trans;\n        }\n        *new_key = key;\n        new_key->name = kstrdup(key.name, GFP_KERNEL);\n        if (!new_key->name) {\n            pr_err("add_filename_trans: kstrdup name failed\\n");\n            goto free_key;\n        }\n        trans->otype = def->value;\n        if (hashtab_insert(db->filename_trans, new_key, trans)) {\n            pr_err("add_filename_trans: hashtab_insert failed\\n");\n            goto free_name;\n        }\n    } else {\n        trans->otype = def->value;\n    }\n\n    return ebitmap_set_bit(&db->filename_trans_ttypes, src->value - 1, 1) == 0;\n\nfree_name:\n    kfree(new_key->name);\nfree_key:\n    kfree(new_key);\nfree_trans:\n    kfree(trans);\nout:\n    return false;\n}\n\n'
    replacements.append(("add_filename_trans", old_f2, new_f2))

    old_f3 = 'static bool add_type(struct policydb *db, const char *type_name, bool attr)\n{\n    struct type_datum *type = symtab_search(&db->p_types, type_name);\n    if (type) {\n        pr_warn("Type %s already exists\\n", type_name);\n        return true;\n    }\n\n    u32 value = ++db->p_types.nprim;\n    type = (struct type_datum *)kzalloc(sizeof(struct type_datum), GFP_KERNEL);\n    if (!type) {\n        pr_err("add_type: alloc type_datum failed.\\n");\n        return false;\n    }\n\n    type->primary = 1;\n    type->value = value;\n    type->attribute = attr;\n\n    char *key = kstrdup(type_name, GFP_KERNEL);\n    if (!key) {\n        pr_err("add_type: alloc key failed.\\n");\n        return false;\n    }\n\n    if (symtab_insert(&db->p_types, key, type)) {\n        pr_err("add_type: insert symtab failed.\\n");\n        return false;\n    }\n\n    struct ebitmap *new_type_attr_map_array =\n        ksu_kvrealloc(db->type_attr_map_array, value * sizeof(struct ebitmap),\n                      (value - 1) * sizeof(struct ebitmap));\n\n    if (!new_type_attr_map_array) {\n        pr_err("add_type: alloc type_attr_map_array failed\\n");\n        return false;\n    }\n\n    struct type_datum **new_type_val_to_struct =\n        ksu_kvrealloc(db->type_val_to_struct,\n                      sizeof(*db->type_val_to_struct) * value,\n                      sizeof(*db->type_val_to_struct) * (value - 1));\n\n    if (!new_type_val_to_struct) {\n        pr_err("add_type: alloc type_val_to_struct failed\\n");\n        return false;\n    }\n\n    char **new_val_to_name_types =\n        ksu_kvrealloc(db->sym_val_to_name[SYM_TYPES], sizeof(char *) * value,\n                      sizeof(char *) * (value - 1));\n    if (!new_val_to_name_types) {\n        pr_err("add_type: alloc val_to_name failed\\n");\n        return false;\n    }\n\n    db->type_attr_map_array = new_type_attr_map_array;\n    ebitmap_init(&db->type_attr_map_array[value - 1]);\n    ebitmap_set_bit(&db->type_attr_map_array[value - 1], value - 1, 1);\n\n    db->type_val_to_struct = new_type_val_to_struct;\n    db->type_val_to_struct[value - 1] = type;\n\n    db->sym_val_to_name[SYM_TYPES] = new_val_to_name_types;\n    db->sym_val_to_name[SYM_TYPES][value - 1] = key;\n\n    int i;\n    for (i = 0; i < db->p_roles.nprim; ++i) {\n        ebitmap_set_bit(&db->role_val_to_struct[i]->types, value - 1, 1);\n    }\n\n    return true;\n}\n\n'
    new_f3 = 'static bool add_type(struct policydb *db, const char *type_name, bool attr)\n{\n    struct type_datum *type = symtab_search(&db->p_types, type_name);\n    if (type) {\n        pr_warn("Type %s already exists\\n", type_name);\n        return true;\n    }\n\n    u32 value = ++db->p_types.nprim;\n    type = (struct type_datum *)kzalloc(sizeof(struct type_datum), GFP_KERNEL);\n    if (!type) {\n        pr_err("add_type: alloc type_datum failed.\\n");\n        return false;\n    }\n\n    type->primary = 1;\n    type->value = value;\n    type->attribute = attr;\n\n    char *key = kstrdup(type_name, GFP_KERNEL);\n    if (!key) {\n        pr_err("add_type: alloc key failed.\\n");\n        return false;\n    }\n\n    if (symtab_insert(&db->p_types, key, type)) {\n        pr_err("add_type: insert symtab failed.\\n");\n        return false;\n    }\n\n    /* --- pre-5.1 compat path: this kernel\'s type_attr_map_array,\n     * type_val_to_struct_array, and sym_val_to_name are all\n     * "struct flex_array *" (confirmed against this kernel\'s own\n     * policydb.h), not the plain kvrealloc\'d arrays dev-susfs assumes.\n     * flex_array is not extensible in place, so a new, bigger one has to\n     * be allocated and the old contents copied across — restored from\n     * KernelSU-Next-susfs-3.2.0\'s own <5.1.0, non-Huawei branch, using the\n     * exact same flex_array_alloc/prealloc/get/put/free calls this\n     * kernel\'s own avtab.c (in security/selinux/ss) already uses for these same\n     * fields. --- */\n    struct flex_array *new_type_attr_map_array =\n        flex_array_alloc(sizeof(struct ebitmap), db->p_types.nprim,\n                         GFP_KERNEL | __GFP_ZERO);\n\n    struct flex_array *new_type_val_to_struct =\n        flex_array_alloc(sizeof(struct type_datum *), db->p_types.nprim,\n                         GFP_KERNEL | __GFP_ZERO);\n\n    struct flex_array *new_val_to_name_types =\n        flex_array_alloc(sizeof(char *), db->symtab[SYM_TYPES].nprim,\n                         GFP_KERNEL | __GFP_ZERO);\n\n    if (!new_type_attr_map_array) {\n        pr_err("add_type: alloc type_attr_map_array failed\\n");\n        return false;\n    }\n\n    if (!new_type_val_to_struct) {\n        pr_err("add_type: alloc type_val_to_struct_array failed\\n");\n        return false;\n    }\n\n    if (!new_val_to_name_types) {\n        pr_err("add_type: alloc val_to_name failed\\n");\n        return false;\n    }\n\n    // preallocate so we don\'t have to worry about the put ever failing\n    if (flex_array_prealloc(new_type_attr_map_array, 0, db->p_types.nprim,\n                            GFP_KERNEL | __GFP_ZERO)) {\n        pr_err("add_type: prealloc type_attr_map_array failed\\n");\n        return false;\n    }\n\n    if (flex_array_prealloc(new_type_val_to_struct, 0, db->p_types.nprim,\n                            GFP_KERNEL | __GFP_ZERO)) {\n        pr_err("add_type: prealloc type_val_to_struct_array failed\\n");\n        return false;\n    }\n\n    if (flex_array_prealloc(new_val_to_name_types, 0,\n                            db->symtab[SYM_TYPES].nprim,\n                            GFP_KERNEL | __GFP_ZERO)) {\n        pr_err("add_type: prealloc val_to_name_types failed\\n");\n        return false;\n    }\n\n    int j;\n    void *old_elem;\n    // copy the old data or pointers to new flex arrays\n    for (j = 0; j < db->type_attr_map_array->total_nr_elements; j++) {\n        old_elem = flex_array_get(db->type_attr_map_array, j);\n        if (old_elem)\n            flex_array_put(new_type_attr_map_array, j, old_elem,\n                           GFP_KERNEL | __GFP_ZERO);\n    }\n\n    for (j = 0; j < db->type_val_to_struct_array->total_nr_elements; j++) {\n        old_elem = flex_array_get_ptr(db->type_val_to_struct_array, j);\n        if (old_elem)\n            flex_array_put_ptr(new_type_val_to_struct, j, old_elem,\n                               GFP_KERNEL | __GFP_ZERO);\n    }\n\n    for (j = 0; j < db->symtab[SYM_TYPES].nprim; j++) {\n        old_elem = flex_array_get_ptr(db->sym_val_to_name[SYM_TYPES], j);\n        if (old_elem)\n            flex_array_put_ptr(new_val_to_name_types, j, old_elem,\n                               GFP_KERNEL | __GFP_ZERO);\n    }\n\n    // store the pointer of old flex arrays first, when assigning new ones we\n    // should free it\n    struct flex_array *old_fa;\n\n    old_fa = db->type_attr_map_array;\n    db->type_attr_map_array = new_type_attr_map_array;\n    if (old_fa) {\n        flex_array_free(old_fa);\n    }\n\n    ebitmap_init(flex_array_get(db->type_attr_map_array, value - 1));\n    ebitmap_set_bit(flex_array_get(db->type_attr_map_array, value - 1),\n                    value - 1, 1);\n\n    old_fa = db->type_val_to_struct_array;\n    db->type_val_to_struct_array = new_type_val_to_struct;\n    if (old_fa) {\n        flex_array_free(old_fa);\n    }\n    flex_array_put_ptr(db->type_val_to_struct_array, value - 1, type,\n                       GFP_KERNEL | __GFP_ZERO);\n\n    old_fa = db->sym_val_to_name[SYM_TYPES];\n    db->sym_val_to_name[SYM_TYPES] = new_val_to_name_types;\n    if (old_fa) {\n        flex_array_free(old_fa);\n    }\n    flex_array_put_ptr(db->sym_val_to_name[SYM_TYPES], value - 1, key,\n                       GFP_KERNEL | __GFP_ZERO);\n\n    int i;\n    for (i = 0; i < db->p_roles.nprim; ++i) {\n        ebitmap_set_bit(&db->role_val_to_struct[i]->types, value - 1, 1);\n    }\n\n    return true;\n}\n\n'
    replacements.append(("add_type", old_f3, new_f3))

    old_f4 = "    struct ebitmap *sattr = &db->type_attr_map_array[type->value - 1];\n"
    new_f4 = "    struct ebitmap *sattr = flex_array_get(db->type_attr_map_array, type->value - 1);\n"
    replacements.append(("add_typeattribute_raw", old_f4, new_f4))

    for name, old, new in replacements:
        if content.count(old) != 1:
            raise SystemExit(f"sepolicy.c: expected exactly one match for {name}, found {content.count(old)}, refusing to patch blindly")
        content = content.replace(old, new, 1)
        print(f"Patched {name}")



    # --- also wrap ksu_destroy_sepolicy/ksu_dup_sepolicy at the end of the
    # file: they operate on struct selinux_policy directly (same finding as
    # rules.c above), and are genuinely dead code on this kernel since
    # nothing in the compiled path calls them here (rules.c's calls are
    # gated the same way, and feature/selinux_hide.c — the only other
    # caller — is stubbed). Gate them instead of leaving an unconditional
    # compile error.
    ...
    if content.count(dead_block) == 1 and content.rstrip().endswith(dead_block.rstrip()):
        wrapped = (
            "#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0)\n"
            + dead_block
            + "#endif // LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0)\n"
        )
        content = content.replace(dead_block, wrapped, 1)
        print("Wrapped ksu_destroy_sepolicy/ksu_dup_sepolicy in a version guard")
    elif "#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0)" in content and "ksu_dup_sepolicy" in content:
        print("ksu_destroy_sepolicy/ksu_dup_sepolicy already wrapped, skipping")
    else:
        raise SystemExit("sepolicy.c: could not find ksu_destroy_sepolicy/ksu_dup_sepolicy block at expected location, refusing to patch blindly")

    with open(path, "w") as f:
        f.write(content)
    print("Patched drivers/kernelsu/selinux/sepolicy.c for this kernel's pre-5.1/5.7/5.8 policydb layout")

PYEOF
if [ $? -ne 0 ]; then
  echo "!!! BUILD ABORTED: failed to patch selinux/sepolicy.c."
  exit 1
fi
